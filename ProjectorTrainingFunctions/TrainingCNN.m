function [net, info] = TrainingCNN(net, imdb, getBatch, varargin)
...
function err = error_euclideanloss(~, labels, res)
% -------------------------------------------------------------------------
...
for t=1:opts.batchSize:numel(subset)
    ...
    for s=1:opts.numSubBatches
        ...
        if strcmp(mode,'val')
            [tmp_snr_inp,tmp_snr_rec]=displayImg(labels,im,res,lowRes,opts);
            snr_inp_set=[snr_inp_set tmp_snr_inp];
            snr_rec_set=[snr_rec_set tmp_snr_rec];
        end
...
display(['avg SNR (inp) ' num2str(mean(snr_inp_set))])
display(['avg SNR (rec) ' num2str(mean(snr_rec_set))])
...
function [snr_inp,snr_rec]=displayImg(labels,im,res,lowRes,opts)
...
subplot(1,3,3); imagesc(recTemp(:,:,1,iter)); colormap(gray); axis off image;title(['Output ' num2str(snr_rec(iter))]);
drawnow;
