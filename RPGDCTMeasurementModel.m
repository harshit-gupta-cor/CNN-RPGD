...
for NoisePoisson=[1 ]
SNRinputVector=[20 25 30 35];
measurementModel=1;GTModel= ~(measurementModel);

   
bvector=1.66*1e4*10.^(( SNRinputVector-15)./10);
maxIter=50 ;
opts.gpuMode=1;
testSet=[328:352];
...
Tolerance=1;
regsnr=@(oracle,rec) computeRegressedSNR(rec,oracle);%b Computes regressed SNR
...
for downSamplingfactor=downSamplingVector
    ...
    s = rng;
    rng(1);
    viewsDownShift = viewslv +doShift*GTModel* randn(size(viewslv)) * 0.05;% Perturbed views for GTModel
    rng(s);
    
    %% Deafult operators
    if defaultOperators
        normalizingConstant=computeConstant(YSize, viewslv,N);
        GToperator=@(x)iradon(x, viewsfv-90, 'linear', 'Ram-Lak', 1, N);
        Alv=@(x)iradon(x,viewslv-90,'linear', 'Ram-Lak', 1, N);
        H=@(x)radon(x,viewslv-90);
        Hpert=@(x)radon(x,viewsDownShift-90);
        HT=@(x)normalizingConstant*iradon(x,viewslv-90,'linear', 'Ram-Lak', 1, N);
    end
    %%
    ...
    for SNRinput=SNRinputVector
        ...
        % and noiselevel
         cd('/Users/Harshit/DeepProject/cnn-pgd')
       b=1.66*1e4*10.^(( SNRinput-15)./10);
        [GammaVector , CVector]=loadCandStep(SNRinput,downSamplingfactor);
        
        noise=logical(1/SNRinput);
        [Addresses,net,FBPconvnet]=loadNettesting(downSamplingfactor,SNRinput,NoisePoisson);
        
        net_gpu = vl_simplenn_move(net, 'gpu') ;
        FBPconvnet_gpu = vl_simplenn_move(FBPconvnet, 'gpu') ;
            ...
            for Gamma=GammaVector
               ...
               for iteration=1:TestSize
                    ...
                    if NoisePoisson
                        ...
                        sino(:,:)=YGT./ScalingNumber(testSet(iteration));
                        lambda =b.*exp(-sino);
                        PhotonCount = poissrnd(lambda);
                        Y =ScalingNumber(testSet(iteration)).* -log(PhotonCount./b);
                        PoissonSNR=snr(YGT,YGT-Y)
                       SNR.Poisson(iteration)=PoissonSNR;
                    ...
                    %% FBPConvnet
                    var.x(:,:,1)= Alv(Y);
                    res=vl_simplenn_fbpconvnet_recursive(FBPconvnet_gpu,gpuArray(single(var.x(:,:,1) )));
                    xFBPconvnet=var.x(:,:,1)+gather(res(end-1).x);
                    SNR.xFBPconvnet=regsnr(GT,xFBPconvnet)*ones(1,maxIter+1);
                    SNR.xsinoFBPconvnet=regsnr(YGT,H(xFBPconvnet))*ones(1,maxIter+1);
                    %% 
                    if NoisePoisson
                        Gradient=@(x) Gamma*HT(((PhotonCount.*H(x)-sqrt(PhotonCount).*Y)))./norm(PhotonCount,'fro');
                    else
                    ...
                    for k=1:maxIter
                        ...
                        res=vl_simplenn_fbpconvnet_recursive(net_gpu,gpuArray(single(v )));
                        ...
                        % Calculate SNRs
                        SNR= GroupSNR(var.x(:,:,k),var.z(:,:,k),k,GT,YGT,H,regsnr,SNR);
                        plotfigure(var.alpha,var.delta,k,SNR,var.x(:,:,k),minI,maxI)
                       
                        if ((var.delta(k)<Tolerance)&& (k>1))
                    ...
                    pathSave=fullfile(address, ['rho_',num2str(fix(rho)),'_Iter_',num2str(maxIter),'_C_',num2str(C)],...
                    ...
                    ssimRPGD = ssim(double(xRPGD), Mag(GT)) ;
                    ssimFBP = ssim(double(xFBP), Mag(GT)) ;
                    ssimFBPconv = ssim(double(xFBPconvnet), Mag(GT)) ;
